...
- [x] Implémenter et comparer différents modèles de classification d’images (CNN, transfer learning)
- [x] Définir les métriques d’évaluation pertinentes
- [x] Entraîner, valider et comparer les performances des modèles

### 4. Analyse des résultats et interprétabilité
- [x] Analyser les erreurs principales et les limites du modèle
- [x] Utiliser des méthodes d’interprétabilité pour valider les prédictions

### 5. Validation sur échantillons indépendants
- [ ] Tester le modèle sur de nouveaux jeux de données
...
- [x] Adapter les notebooks/scripts à la structure et aux formats réels
- [x] Enrichir les métriques et visualisations post-entraînement
- [x] Implémenter des callbacks pour logger MCC et balanced accuracy
...
| 2025-07-06 | Équilibrage des classes           | Oversampling dynamique de la classe minoritaire dans `train_df` + recalcul de la colonne `weights` | Fait | Classes équilibrées, recall toxic ↑ |
| 2025-07-06 | Pipeline de données `tf.data`     | Passage d’`ImageDataGenerator` à un pipeline `tf.data` (TIFF support, augmentations fortes, prefetch) | Fait | Lecture 25 % plus rapide, mémoire stable |
| 2025-07-06 | Métriques avancées & callback     | Mise à jour de `src/metrics.py` (ROC/PR, MCC, calibration) + ajout du callback `F1MCCCallback` | Fait | `val_mcc` et `val_bal_acc` loggés chaque époque |
| 2025-07-06 | Gestion warning Keras             | Suppression du warning « PyDataset class should call super().__init__ » via `weight_col` | Fait | Warning disparu, pipeline propre |
| 2025-07-06 | Focal Loss & callbacks            | Adoption de `BinaryFocalCrossentropy` + EarlyStopping/ModelCheckpoint sur `val_pr_auc` | Fait | AUC-PR ↑, overfitting réduit |
...
| 2025-07-08 | Développement de modèle        | Mise à jour de `05_finetune_resnet18.ipynb` (ajout ROC AUC, courbes PR, early-stopping, calibration)                                                                                                                                                                           | Fait     | Validation enrichie avec AUC/PR et contrôle d’overfitting        |
| 2025-07-08 | Développement de modèle        | Expérience `data/raw/all et` `data/raw/Switzerland` avec `05_finetune_resnet18.ipynb`                                                                                                                                                                                              | Fait     | Performances similaires          |
| 2025-07-10 | Grad-CAM & interprétabilité       | Intégration de `pytorch-grad-cam`, heatmaps HD en grille | Fait | Visualisation claire, code plus concis |
| 2025-07-10 | Évaluation calibrée ResNet18      | Early-stopping + calibration Platt, nouvelle évaluation (Val acc 0.983, AUC/AP 1.00) | Fait | Modèle fiable, seuil optimal conservé |
| 2025-07-10 | Reproductibilité                  | Documentation et activation des options déterministes (seeds, `TF_DETERMINISTIC_OPS`) | Fait | Résultats reproductibles sur GPU |
...
### 2025-07-09 — Déséquilibre de classes & rappel toxique instable

**Problème :**  
Oscillations extrêmes de `val_recall_toxic` dues au fort déséquilibre (toxic ≫ non-toxic).

**Solution :**  
- Oversampling de la classe minoritaire jusqu’à équilibre.  
- Re-génération de `weights`.  
- Calcul automatique du seuil optimisant la F1 via la PR-curve.

### 2025-07-10 — Non-déterminisme GPU / CuDNN

**Problème :**  
Variabilité des résultats entre runs (kernels GPU non déterministes, multithreading TF).

**Solution :**  
- Fixer les seeds `random`, `numpy`, `tensorflow`.  
- Export `TF_DETERMINISTIC_OPS=1`, limiter `intra/interop threads`.  
- Utiliser les API *stateless* et réduire le parallélisme du pipeline `tf.data`.
